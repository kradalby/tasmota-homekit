...
- Dev servers must advertise distinct names: set `TASMOTA_HOMEKIT_BRIDGE_NAME` and `TASMOTA_HOMEKIT_TS_HOSTNAME` to a `*-dev` variant so they never collide with production HomeKit/Tailscale identities.
